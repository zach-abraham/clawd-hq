...
|--------------|-----------------------|----------------------------------------|
| Mac Mini     | Primary compute       | 8 agents, Docker stack, OpenClaw, A2A  |
| MacBook Air  | Mobile development    | Claude Code, Chrome debug profile      |
| Gaming PC    | GPU workloads         | Model training, batch inference        |
| VPS          | Production automation | Job automation, public-facing services |
| Router       | Network edge          | AdGuard Home, Tailscale subnet router  |

### Network Addressing

All inter-machine communication uses Tailscale. Services bind to the mesh interface and are accessible from any machine on the network. No ports are exposed to the public internet.
...
<plist version="1.0">
  <dict>
    ...
    <array>
      ...
      <string>$PROJECT_ROOT/<agent>/scripts/run.sh</string>
    </array>
...
|---------------|------------|-------------------------------|
| Uptime Kuma   | 1          | `louislam/uptime-kuma:1`      |
| Beszel Hub    | 1          | `henrygd/beszel`              |
| Langfuse      | 6          | `langfuse/langfuse` + deps    |
| n8n           | 1          | `n8nio/n8n`                   |
...
```
GET http://<mesh-ip>:<port>/.well-known/agent.json

{
  "name": "HAL",
  "description": "Clawd HQ orchestrator agent",
  "url": "http://<mesh-ip>:<port>",
...
An iMessage gateway runs on the Mac Mini, providing API access for SMS verification during automated signups.
...
```
1. Uptime Kuma API  --> Are all services responding?
2. Beszel API       --> Any machine above resource thresholds?
